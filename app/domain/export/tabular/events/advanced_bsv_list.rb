...
module Export::Tabular::Events
  class AdvancedBsvList < BsvList
    ...
    def add_counts(labels)
      ...
      labels[:bsv_eligible_participations_count] = 'alle berechtigten Personen (bis 30)'
      labels[:bsv_eligible_attendance_summary] = 'alle berechtigten Personen (bis 30) x Tage'
      labels[:bsv_eligible_attendances] = 'Berechtigte Tage'
      labels[:leader_count] = 'Leitungsteam inkl. Küche'
      labels[:all_participants_count] = 'alle Personen unabhängig vom Alter'
      labels[:all_participants_attendance_summary] = 'alle Personen unabhängig vom Alter x Tage'
