...
describe GroupsPbsHelper do
  ...
  context "#format_group_website" do
    ...
    it "adds http if missing" do
      ...
      expect(subject).to eq('<a target="_blank" rel="noopener" href="http://schekka.ch">schekka.ch</a>')
    end

    it "keeps http if present" do
      group.website = "https://schekka.ch"
      expect(subject).to eq('<a target="_blank" rel="noopener" href="https://schekka.ch">https://schekka.ch</a>')
    end
  end
end
