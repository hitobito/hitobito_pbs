...
describe Event::ParticipationsController do
  ...
  context 'GET#index' do
    ...
    it 'does not include tentative participants' do
      Fabricate(:event_participation, event: course, state: 'applied', person: people(:child), active: true)
  ...
  context 'POST#create_tentative' do
    ...
    it 'raises CanCan::AccessDenied when course does not support tentative applications' do
      expect do
        post :create_tentative, group_id: group.id, event_id: course.id, event_participation: { person_id: people(:al_schekka).id }
    ...
    it 'raises CanCan::AccessDenied when person is not accessible' do
      ...
      expect do
        post :create_tentative, group_id: group.id, event_id: course.id, event_participation: { person_id: people(:child).id }
      end.to raise_error CanCan::AccessDenied
    end
