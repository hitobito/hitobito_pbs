...
describe RolesController do
  ...
  let(:role_defaults) {  { group_id: group.id,
                           person_id: person.id,
  ...
  describe 'POST #create' do
    ...
    context 'with valid params' do
      let(:role_params) { role_defaults.merge(created_at: Date.new(2014,3,4),
    ...
    context 'with deleted before created at' do
      let(:role_params) { role_defaults.merge(deleted_at: Date.new(2014,3,4),
