...
describe Event::ParticipationMailer do
  ...
  context "canceled" do
    ...
    context "headers" do
      ...
      its(:subject) { is_expected.to eq "Kursabmeldung" }
      its(:to) { is_expected.to eq ["bulei@hitobito.example.com"] }
    ...
    context "body" do
      ...
      it "renders placeholders" do
        is_expected.to match(
          /AL Schekka \/ Torben wurde per 01.07.2015 vom Kurs Top Course abgemeldet./
        )
      end
    end
  end
  ...
  context "rejected" do
    ...
    context "headers" do
      ...
      its(:subject) { is_expected.to eq "Kursablehnung" }
      its(:to) { is_expected.to eq ["bulei@hitobito.example.com"] }
      its(:from) { is_expected.to eq ["noreply@localhost"] }
    end
      ...
      before :all do
        ...
        template.update!(
          ...
          body: "{recipient-name-with-salutation}<br/><br/>" \
                ...
                "{event-details}<br/><br/>" \
                "Falls du ein Login hast, kannst du deine Anmeldung unter folgender Adresse " \
                " einsehen und eine Bestätigung ausdrucken:<br/><br/>{application-url}"
        )
      end
